...
export default props => {
  const { name, symbol, price, open, high, low, previousClose } = props.stock
  ...
  let changeClass = cn("mr-1", {
    "text-green": positive,
    "text-red": !positive
  })

  let arrowClass = cn("fill-current inline w-3 mr-1 svg-fix", {
    "rotate-180": !positive
  })
      ...
      <div class="sm:inline-block sm:w-60 lg:w-80 w-full h-full p-3 bg-white">
        <div class="sm:block flex sm:flex-wrap items-baseline justify-between relative">
          <div class="sm:block truncate">
            ...
            <span class="block text-gray-light">{symbol}</span>
          </div>
          <div class="relative right-0 text-right sm:mb-2 sm:text-left">
            ...
            <span class="block sm:inline-block w-32">
              ...
              <span class={percentClass}>({round(percentage, 2)}%)</span>
            </span>
          </div>
          <div class="hidden sm:flex flex-wrap">{priceList}</div>
        </div>
      </div>
    </div>
  )
